...
export const useSandboxStore = create<SandboxStore>()(set => ({
    ...
    chatStatus: 'ready',
    clearGeneratedFiles: () =>
        set(() => ({ generatedFiles: new Set<string>() })),
    commands: [],
    currentView: 'code',
    ...
    upsertCommand: cmd => {
        set(state => {
            ...
            const idx =
                existingIdx !== -1 ? existingIdx : state.commands.length;
            const prev = state.commands[idx] ?? {
                startedAt: Date.now(),
                logs: [],
            };
...
export const useSetChatStatus = () =>
    useSandboxStore(state => state.setChatStatus);
export const useSetCurrentView = () =>
    useSandboxStore(state => state.setCurrentView);
export const useSandboxId = () => useSandboxStore(state => state.sandboxId);
export const useGeneratedFiles = () =>
    useSandboxStore(state => state.generatedFiles);
...
export const useSandboxUrl = () =>
    useSandboxStore(state => ({ url: state.url, urlUUID: state.urlUUID }));
